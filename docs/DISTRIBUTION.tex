...
\subsection{Build backend: hatchling with a binary-staging hook}

The build backend is \href{https://hatch.pypa.io/latest/}{hatchling}. The
package is \emph{pure Python}: there is no Rust source in this repo and no
Python extension module. The platform-specific \pkg{hawcx-manager} binary is
staged as package data rather than compiled --- the release workflow drops it
into \code{src/hawcx\_haap/\_bin/} before each wheel build, and the custom
hatchling hook in \code{python/hatch\_build.py} force-includes that directory
\emph{only when it exists}.
...
Consequences: a dev or editable install needs no Rust toolchain and no cargo
registry access (the hook no-ops, and \code{get\_binary\_path()} raises at call
time instead); the hook emits a \code{py3-none-any} wheel that
\code{release-python.yml} retags with \code{python -m wheel tags}; and platform
wheels must be built with \code{python -m build --wheel}, never via the sdist,
which deliberately excludes \code{\_bin}.

\emph{Correction (2026-08-22): this section previously described maturin with}
\code{bindings = "bin"}\emph{. That was accurate before the alpha.13 packaging
change and no longer describes the repo.}
...
(e.g., installed from an sdist or an editable checkout, where nothing was
staged into \code{\_bin/}). The error message explains the local development
workflow.
